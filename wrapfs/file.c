...
static void xcfs_encrypt (const char *buff, char *tmp, int size){
	  int i =0;
		for (i=0; i<size;i++){
       *tmp = *buff + 1;
       buff++;
...
static void xcfs_decrypt ( char *buff, int size){
	int i =0;
	while (i!= size){
		*buff -= 1;
		buff++;
			    ...
			    size_t count, loff_t *ppos)
{
  ...
  char *tmp = kmalloc(count, GFP_KERNEL);
	...
	lower_file = wrapfs_lower_file(file);
   xcfs_encrypt(buf, tmp,  count);
//	err = vfs_write(lower_file, tmp, count, ppos);
	
  err = kernel_write(lower_file, tmp, count , *ppos);
